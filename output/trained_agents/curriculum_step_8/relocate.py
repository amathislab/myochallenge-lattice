...
class CustomRelocateEnvPhase2(RelocateEnvV0):  #RelocateEnvV0Phase2
    ...
    def get_reward_dict(self, obs_dict):
        ...
        reach_dist = np.abs(np.linalg.norm(self.obs_dict['reach_err'] + np.array([0., 0., self.reach_z_offset]), axis=-1))
        pos_dist_offset = np.abs(np.linalg.norm(self.obs_dict['pos_err'] + np.array([0., 0., self.pos_z_offset]), axis=-1))
        ...
        reach_dist_z = np.abs(np.linalg.norm(self.obs_dict['reach_err'][:,:,2] + self.reach_z_offset, axis=-1))
        ...
        rwd_dict = collections.OrderedDict((
            ...
            ('solved', np.array([[(pos_dist < self.pos_th).item() and (rot_dist < self.rot_th).item() and (not drop)]]) ),

...
class CustomRelocateEnvPhase2(RelocateEnvV0):  #RelocateEnvV0Phase2
    ...
    def _setup(self,
        ...
        self.init_obj_z = 0
        self.goal_obj_z = np.zeros([1,1,3]) #0 #np.zeros([1,1,3])
    ...
    def get_reward_dict(self, obs_dict):
        ...
        pos_dist_obj_z = np.abs(np.linalg.norm(self.obs_dict['obj_pos'] - self.goal_obj_z, axis=-1))
    ...
    def reset(self, reset_qpos=None, reset_qvel=None):
        ...
        self.goal_obj_z = np.array(self.sim.data.site_xpos[self.object_sid]) + np.array([0., 0., 0.3])
